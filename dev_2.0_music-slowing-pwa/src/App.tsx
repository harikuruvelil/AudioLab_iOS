...
export default function App() {
  ...
  const [bgBassLow, setBgBassLow] = useState<number>(
    initialAudioSettings?.bgBassLow ?? 40
  );
  const [bgBassHigh, setBgBassHigh] = useState<number>(
    initialAudioSettings?.bgBassHigh ?? 280
  );
  const [bgBassThreshold, setBgBassThreshold] = useState<number>(
    initialAudioSettings?.bgBassThreshold ?? 0.02
  );
